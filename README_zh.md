...
- **JDK 17+** — 运行 MCP Server 必需
- **Maven 3.6+** — 构建项目必需
- 目标项目的依赖已下载到本地仓库（`~/.m2/repository`），如未下载可在目标项目中执行 `mvn dependency:resolve`
...
构建产物为 fat jar：`target/maven-lens-1.0.0.jar`

## MCP 配置

Maven Lens 以 **STDIO 模式**运行 — AI 客户端将 jar 作为子进程启动，通过 stdin/stdout 通信。
...
> 将 `/absolute/path/to/maven-lens-1.0.0.jar` 替换为实际的 jar 文件绝对路径。
